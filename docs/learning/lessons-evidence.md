...
| 2026-08-02 extension | The final pacing run exposed that a later car/pedestrian split left the hard-coded 88-car post-load expectation stale even though the fixture SHA was unchanged. Fix commit: (this commit). `tests/performance/performance-fixture-contract.test.ts` now loads the real fixture with the shared shipping-worker flags, rebuilds derived state, advances once, and pins the exact 936-person / 453-building / 81-car / 17-pedestrian world and projection. Both browser drivers consume the same contract and report the actual state on drift. Behavior delta: a simulation change can no longer leave a byte-identical performance fixture paired with a stale rendered-workload claim. |
...
## `npm audit fix` can report a fix it never applies

| Field | Value |
|---|---|
| Surfaced by | A session-start gate sweep found `npm audit --audit-level=high` red on the full tree with three high advisories — `brace-expansion`, `nanoid`, `postcss` — while `--omit=dev` was clean. |
| Reviewer findings | `npm audit` printed "fix available via `npm audit fix`" for all three, but `npm audit fix --package-lock-only --dry-run` answered "up to date, audited 146 packages" and left `package-lock.json` byte-identical. It also did not print the "including breaking changes, run `npm audit fix --force`" line that normally explains a refusal, so the output gave no signal that nothing had happened. All three packages were genuinely in this repo's own lockfile and `node_modules`, not only in the linked `file:` siblings, and a satisfying fixed version existed inside each parent's declared range. |
| Fix commit | `7c2e8e0` |
| Test added | None — the gate is `npm audit --audit-level=high` on both trees, which already fails closed. |
| Behavior delta | `npm update brace-expansion nanoid postcss --package-lock-only` re-resolved all three (5.0.7 → 5.0.9, 3.3.15 → 3.3.18, 8.5.16 → 8.5.26) with no `package.json` change and no parent bump, and both audit trees went to 0 vulnerabilities. Rule: when `npm audit fix` claims a fix, confirm it by diffing `package-lock.json` rather than by re-reading the audit summary — a no-op reports the same way as a refusal. Reach for `npm update <pkg> --package-lock-only` to force re-resolution of a transitive pin. Worth knowing separately: the same write refreshed the `../voxel` link metadata from a stale 0.1.4 to its current 1.2.0, so a lockfile touched at all will also absorb whatever the linked siblings have become since the last install. |

## A test driven through the system it tests can pass while the defect is live

| Field | Value |
|---|---|
| Surfaced by | Fixing review finding 3 (`schoolFor` picking a nearer school on an unreachable road component). The new contract test drove the trip system for a full day and asserted the child walked to the reachable school. |
| Reviewer findings | The test passed with the component filter DELIBERATELY DISABLED. An unreachable choice does not announce itself: `findRoadCellPath` returns null, the member loop `continue`s, and no walker is ever created — which is indistinguishable from "the morning departure moment has not arrived yet". The assertion only ran inside `if (slot?.place != null)`, so with the defect live the loop simply never entered the branch and the final "did it attend" check was reached by a path the fixture happened to satisfy. Two earlier tests in the same session also failed for the wrong reason first (`sim.world.getPosition` does not exist; `citizenDetail` takes `(sim, entity)`, not `(sim, world, entity)`), and a third asserted a `place` object against a raw entity id. |
| Fix commit | `84d2cc8` |
| Test added | `tests/sim/school-runs.test.ts` — "does not let a nearer unreachable school shadow a reachable one" now asserts `schoolFor(sim, home)` DIRECTLY, and its guards assert the decoy is genuinely nearer (5 vs 10) and genuinely on another road component before relying on either. |
| Behavior delta | With the filter disabled the rewritten test fails (picks 46, wants 32); with it enabled the suite is green. Rule: when the defect's signature is ABSENCE — no agent, no event, no row — do not assert it through the system that would have produced the thing. Assert the deciding function directly, and add a guard that fails loudly if the fixture stops exercising the case. Corollary applied throughout this review: confirm a new test is RED *for the reason you intended* before writing the fix, and after fixing, re-break the fix and watch the test fail — a green test proves nothing until you have seen it go red for the right reason. |

## A silhouette-joint check must measure the tightest side, not the widest

| Field | Value |
|---|---|
| Surfaced by | The reported defect "tree body is separated from trunk": terrain-tree canopies visibly hovered above their trunks (`trees.ts`, `TREE_ARCHETYPES`). |
| Reviewer findings | The root cause was ordinary — `lift` positioned a canopy by its nominal height box, and `DodecahedronGeometry(1, 0)` reaches only y = -phi/sqrt(3) ~= -0.934, filling 93.4% of that box, so the foliage floated by the difference plus the intended lift. The trap was in the TEST. Its first version measured the widest point of the canopy's cross-section at the trunk top. Re-run against a shallow sink (broadleaf 0.10) it reports 0.158 against a 0.075 trunk radius and PASSES — while the same slice measures 0.040 perpendicular to the faceted bottom edge, which is daylight, and the trees are randomly rotated per cell under a free-orbiting camera. A convex solid tapering to an edge or a point is wide from exactly one azimuth and that is the one a max-radius probe finds. |
| Fix commit | `f2a8505` |
| Test added | `tests/rendering/trees.test.ts` — "seats every canopy on the trunk it grows from, with no daylight at the joint". `narrowestCrossSection()` interpolates every triangle edge straddling the plane into a section polygon and returns its distance to the axis at its tightest side. Both assertions were confirmed load-bearing: every `canopySink` at 0 fails "canopy reaches below the trunk top"; broadleaf at 0.10 clears that and fails the width one (`expected 0.0401898302980177 to be greater than 0.075`). |
| Behavior delta | Narrowest canopy section at the trunk top went 0.000 for all three archetypes to 0.318 / 0.141 / 0.085, against trunk radii 0.055 / 0.075 / 0.060. Rule: when checking that two solids visibly join, measure the joint where it is NARROWEST across the full turn, not where it is widest — the widest reading is taken at the one angle the defect hides behind. The same applies to any low-poly assembly of stacked primitives here (service, utility, and leisure structures), and to any "is A covered by B" geometric claim. Corollary that cost the least and paid the most: seat a mesh by its COMPUTED bounding box rather than by its nominal dimensions, so a new shape cannot silently be seated wrong. |

## A camera you set is not the camera that took the frame

| Field | Value |
|---|---|
| Surfaced by | Building `npm run visual:sweep`, the standing multi-angle visual check. Two of four low-angle frames photographed the underside of the map, while the script's own log said the camera was 0.36 above the ground. |
| Reviewer findings | Two independent overrides sit between `camera.position.set(...)` and the shutter. `MapControls` clamps `minDistance` to 8 and `maxPolarAngle` to π/2.2, so a close-up request is silently answered with an 8-unit view — every "close-up" screenshot taken through the game's controls before this was an 8-unit frame, cropped. And `CityScene` re-pins `controls.target.y` to the terrain **every frame**, shifting the camera by the same delta to preserve the angle: an aim point raised 0.44 above the ground drops the camera 0.44 before the screenshot. Measured by reading the camera at aim time and again at shutter time: set (106.4, 1.22, 41.6), photographed from (106.4, 0.78, 41.6), ground at 0.86. The failure was invisible from the log because the log recorded the requested position. |
| Fix commit | `cb03663` |
| Test added | None — this is a property of the instrument, not the product. `scripts/visual-sweep.mjs` now lifts both clamps for inspection, aims at ground level and expresses the view angle purely as eye height, and records `cameraY` read **after** the settle, alongside the ground height under the camera, in `frames.json`. |
| Behavior delta | Grazing frames unusable: 2 of 4 on the first subject → 0 of 44, minimum measured ground clearance 0.84. Rule: when a script drives a camera (or any live control) and captures a frame later, read the state back at capture time and record it with the artifact — a control loop running between the two will win, and the value you set is not evidence of the value that applied. The same reasoning covers any clamped input: confirm the clamp did not eat the request. |

## A verification scaffold that hardcodes map coordinates reports its own bugs as product failures

| Field | Value |
|---|---|
| Surfaced by | Browser-verifying the abandonment retune: a script that builds a district, neglects it, then "rescues" it with a plant, lines, a pump, and pipes, and asserts the city survives. It printed **FAIL the city was lost despite the rescue** three times, and a fourth failure earlier in the same session claimed a power-plant panel could not be opened. |
| Reviewer findings | Every one of the four was the scaffold, not the game. The map is PROCEDURAL and 128x128; a district pinned at x 20..60, y 30..40 straddled a lake. In order: the plant site was underwater so `utilityCellOwner` was empty and `cell % W` produced `NaN`; a service anchored one row from the road was refused; a power line started ON the plant's own footprint; and the corridor chosen to reach both zone rows crossed water. Each looked exactly like a product defect from the log — a rescue that did not rescue, a panel that would not open. The sim's own error messages are what identified all four ("(44, 36) is water — power lines cannot cross it", "no cell of the footprint at (11, 3) touches a road — services must sit beside one"), which is the error-messages-as-a-product-surface rule paying for itself inside the repo's test scaffolding. The fix is one function: scan for a verified all-land block of the needed size and lay the whole scenario relative to it, exactly as `tests/sim/helpers.ts` `findLandBlock` already does for the sim tests. |
| Fix commit | this session's abandonment-retune commit (the scaffolds themselves are ignored task-run evidence and were deleted) |
| Test added | None — this is a property of throwaway verification scripts, not of the product. The standing rule is below. |
| Behavior delta | 4 false product failures -> 0 once the scenario was placed on a verified block. Rule: a browser or headless scaffold on this map must DERIVE every coordinate — scan for clear land, read the plant footprint back before routing from it, and check the returned `lastCommandSubmission.message` on every placement instead of assuming it took. Treat a scaffold's first "the product is broken" as a claim about the scaffold until its placements are proven accepted. |
