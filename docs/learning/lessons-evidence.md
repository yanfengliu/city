...
## Gates cannot pass from inside an agent worktree because CRLF inflates every pinned byte size

| Field | Value |
|---|---|
| Surfaced by | Fixing the `.claude/**` gate-config gap on 2026-08-02. The plan was to verify inside the agent worktree the session was handed, so `npm test` there was expected to be green apart from the doubling; instead `tests/performance/recorder-benchmark.test.ts` failed on `package.json size drifted — expected 1118 to be 1154`, with no source change to explain it. |
| Reviewer findings | 1154 − 1118 = 36, exactly the 36 lines of `package.json`, so the difference is one byte per line: CRLF against LF. `core.autocrlf` is `true` globally and the repo has no `.gitattributes`, so a freshly added worktree materializes CRLF while the long-lived main checkout is still LF. Measured directly: `package.json` is 1118 bytes in `city/` and 1154 in both `.claude/worktrees/*`. The manifest pins main's LF sizes, so every one of the ~100 pinned files is inflated in a worktree and the check can never pass there — the first mismatch just happens to be `package.json`. |
| Fix commit | None. The trap is documented, not gated: normalizing with a `.gitattributes` would renormalize the whole tree and rewrite the benchmark manifest's pinned sizes, which is its own reviewed change, not a side effect of a lint/vitest config fix. |
| Test added | None — that absence is the reason this is a lesson rather than a deleted one. |
| Behavior delta | Unchanged; what changed is where you run the gates. Run all four from `city/` itself, which is also where the reproduction condition lives, since the worktrees are nested under it. A byte-size drift on a file you did not touch means you are in the wrong checkout, not that the pin is stale — check `wc -c` against `city/` before reaching for `npm run benchmark:recorder`. |
