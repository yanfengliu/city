...
- Zones: a grouped renderer layer with translucent zone tint plus small low-opacity zone-colored planning parcels. Both meshes rebuild from the same `zones` payload and hide cells covered by building or service footprints, so empty planned lots read as civic planning overlays while grown buildings stay visually clean.
