...
- Terrain: one static group from the `ready` terrain: bright model-table grass, clear blue recessed water, vertical shore skirts, and warm sandy shoreline detail strips on real land-water borders (all renderer-only; the sim water mask stays authoritative). Trees: the renderer deterministically partitions the initial tree mask across conifer, broadleaf, and columnar low-poly archetypes, each retained as synchronized instanced trunk/lower/upper layers (at most nine fixed draw calls independent of tree count). Cell hashes choose archetype, coordinated trunk/foliage family, non-uniform proportions, rotation, and a small within-cell offset; road/building/structure occupancy only filters those stable assignments, so bulldozing restores the same tree rather than rerolling it. No extra protocol message or sim state is required.
...
- Field overlays: one 64x64 (or 32x32 for blockSize 4) `DataTexture` per active field on a transparent plane above terrain; updated from sparse `field` messages. Only the active overlay is subscribed. The client-computed power/water overlay mirrors the sim's monotone conduction fixpoint—starting at source/conductor cells and repeatedly attaching in-range live growables plus services—so its connection-reach halo remains accurate during brownouts.
