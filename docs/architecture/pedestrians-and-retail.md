...
The worker projects the full active pedestrian list each tick as `{id, generation, fromCell, toCell, t, purpose, outbound}`. Long paths stay inside the worker. The renderer interpolates one current road-cell segment, offsets it toward the curb according to travel direction, samples the shared terrain surface, and uses purpose color to distinguish commercial work, industrial work, and shopping.

Pedestrians use a fixed-capacity pair of instanced batches for bodies and heads. They do not cast shadows, so moving agents do not invalidate the cached 2048-square shadow map. Empty-list cleanup and entity-generation replacement are explicit, and the global simulation/render cap bounds both ECS and GPU work.
