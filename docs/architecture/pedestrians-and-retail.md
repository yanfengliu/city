...
Pedestrians use three fixed-capacity instanced batches for tops, lower garments, and heads, with a small front cue for facing. They do not cast shadows, so moving agents do not invalidate the cached 2048-square shadow map. Empty-list cleanup and entity-generation replacement are explicit, and the global simulation/render cap bounds both ECS and GPU work.
