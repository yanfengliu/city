...
- Day = 2048 ticks (visual day/night only). Budget interval = 1024 ticks (income and upkeep are defined per budget interval; the budget system runs at interval 1024, offset 1023 — the first budget fires at tick 1024, not tick 1).
...
Toolbar: select/inspect, road, bulldoze, zone R/C/I, dezone, services x4, coal/wind, power line, pump, pipe. Top HUD: treasury (+ per-interval delta), population, RCI demand bars, day counter, speed controls, and a Budget button toggling a top-right panel with the last interval's income/expenses/net and per-zone tax sliders (0–20%, authoritative rates echoed back from sim frame stats). Overlays menu: pollution, noise, land value, traffic (edge congestion coloring), power, and water (network cells, 2-cell connection reach halo, unsupplied buildings in red). Click a building → info panel (zone, level, residents/jobs, powered/watered, score inputs). Toasts appear for rejected commands and save/load results; a HUD warning badge shows broke and disconnected-trips states, and a dismissable scrollable Advisor panel (bottom right) lists prioritized guidance (rows with a problem location are clickable — the camera flies to the building and flashes a highlight; the first occurrence of each problem type expands with step-by-step fix instructions until confirmed with Got it, persisted in localStorage): missing roads/zones/power/water, unsupplied building counts, abandonment, unroutable commuters, budget deficit (last interval's expenses exceeded income while solvent), demand pressure, and unemployment.
