...
- Day = 4096 ticks: one full sun rotation (day/night lighting, renderer-side in `scene.setDayFraction`) equals one day-counter day — ~205 s of real time at 1x, a relaxed cycle. Budget interval = 1024 ticks, independent of day length (income and upkeep are defined per budget interval; the budget system runs at interval 1024, offset 1023 — the first budget fires at tick 1024, not tick 1).
...
Toolbar: select/inspect, road, bulldoze, zone R/C/I, dezone, services x4, coal/wind, power line, pump, pipe. Each tool has a single-key shortcut shown as a badge on its button and wired globally (ignored while a modifier is held or a text field is focused): R/C/I select the zones (the classic mnemonic), so Road takes D and the rest fall to nearby free letters — Select S, Bulldoze B, Dezone X, Fire F, Police P, Clinic H, School E, Coal G, Wind W, Line L, Pump U, Pipe J. Top HUD: treasury (+ per-interval delta), population, RCI demand bars, day counter, speed controls, and a Budget button toggling a top-right panel with the last interval's income/expenses/net and per-zone tax sliders (0–20%, authoritative rates echoed back from sim frame stats). Overlays menu: pollution, noise, land value, traffic (edge congestion coloring), power, and water (network cells, 2-cell connection reach halo, unsupplied buildings in red). Click a building → info panel (zone, level, residents/jobs, powered/watered, score inputs). Toasts appear for rejected commands and save/load results; a HUD warning badge shows broke and disconnected-trips states.

Advisor panel (bottom right): a scrollable, prioritized guidance list. Two row kinds. **Tips** are guided checklists — the onboarding/utility steps (found the city by linking a road to the highway; zone R/C/I then grow a building; place a plant then power every building; place a pump then water every building). Each step shows a green ✓ when its live condition is met (an ○ while pending); a tip has no dismiss button and stays expanded until EVERY step is satisfied, then drops out on its own — so a tip only collapses once all its requirements are done. Until the city is linked to the highway, only the founding tip shows. **Plain advisories** (dismissable, return if the situation persists; clickable ones fly the camera to the problem and flash it): abandonment, unroutable commuters, budget deficit (last interval's expenses exceeded income while solvent), demand pressure, unemployment, and the broke state.
