...
The driver always launches Chromium headless, starts loopback servers on ephemeral ports, and closes both browser and servers in `finally`. A result contains the exact run order, fixture hash/seed, a path/byte/SHA-256 manifest for both served production trees, browser/GPU/host metadata, pooled percentiles, summaries, and every raw per-frame sample. Every generated result is task-local evidence: write it under ignored `output/performance/`, review it locally, retain only concise conclusions and reproducible provenance in tracked docs, and remove the raw file when review ends. Never promote results into the ignored legacy `benchmarks/results/` path, Git, LFS, or a remote artifact store. `npm run benchmark:fixture` generates an ignored candidate at `output/performance/performance-city-save-candidate.json` with the shipping worker flags; it never silently replaces the SHA-pinned canonical save. Promoting a candidate is a reviewed re-earn change that updates the deterministic fixture, shared fixture contract, tests, and concise tracked conclusions together, with `--output benchmarks/fixtures/performance-city-save.json --allow-canonical-overwrite true` reserved for that change.
...
```powershell
npm.cmd run benchmark:recorder -- --source-ref <sha-or-working-tree> --output output\performance\recorder-profile-<source-ref>.json
```

The recorder result follows the same local-only lifecycle: audit it from the ignored output path, copy only its concise aggregate conclusion and provenance into tracked Markdown, then remove the raw profile.
